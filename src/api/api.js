...
export const fetchCardsOfDeck=(deck)=> axios.get(`${card_deck}/${deck.id}`);
export const addCardToDeck=(cardToAdd,deck)=> axios.patch(`${card_deck}/${deck.id}`, cardToAdd);
export const removeCardFromDeck=(card,deck)=> axios.patch(`${card_deck}/${deck.id}`, card);

const verse_tag=backend_url+"/verse_tag";
export const fetchVersesOfTag=(tag)=> axios.get(`${verse_tag}/${tag.id}`);
export const addTagToVerse=(verseToAdd,tag)=> axios.patch(`${verse_tag}/${tag.id}`, verseToAdd);
export const removeTagFromVerse=(verse,tag)=> axios.patch(`${verse_tag}/${tag.id}`, verse);
const tag_verse=backend_url+"/tag_verse";
export const fetchTagsOfVerses=(verse)=> axios.get(`${tag_verse}/${verse.id}`);

const verse_card=backend_url+"/verse_card";
export const fetchVersesOfCard=(card)=> axios.get(`${verse_card}/${card.id}`);
export const addVerseToCard=(verseToAdd,card)=> axios.patch(`${verse_card}/${card.id}`, verseToAdd);
export const removeVerseFromCard=(verse,card)=> axios.patch(`${verse_card}/${card.id}`, verse);
